...
Отдельные хуки для отдельных состояний
Хотя бывают случаи, когда может быть полезно хранить связанные данные в коллекции данных, такой как массив или объект, также может быть полезно разделить данные, которые изменяются отдельно, на совершенно разные переменные состояния. Управлять динамическими данными намного проще, если мы делаем наши модели данных максимально простыми.

Например, если бы у нас был единственный объект, хранящий состояние предмета, который вы изучаете в школе, он мог бы выглядеть примерно так:
...
Это сработает, но подумайте о том, насколько беспорядочным может быть копирование всех других значений, когда нам нужно что-то обновить в этом большом объекте состояния. Например, чтобы обновить оценку на экзамене, нам понадобится обработчик событий, который делал бы что-то вроде этого:
...
Ой! Такой сложный код может вызвать ошибки! К счастью, есть еще один вариант ... Мы можем сделать несколько звонков в State Hook. Фактически, мы можем сделать столько звонков, useState()сколько захотим! Лучше всего разбить состояние на несколько переменных состояния в зависимости от того, какие значения имеют тенденцию изменяться вместе. Мы можем переписать предыдущий пример следующим образом…
...
Управление динамическими данными с помощью отдельных переменных состояния имеет много преимуществ, например, упрощение написания, чтения, тестирования и повторного использования нашего кода в разных компонентах.

Часто мы упаковываем и организуем данные в коллекции для передачи между компонентами, а затем разделяем те же самые данные внутри компонентов, где разные части данных изменяются отдельно. В работе с хуками замечательно то, что у нас есть свобода организовать наши данные так, как это наиболее удобно для нас!

Если хотите, взгляните на другой пример использования нескольких хуков состояний для управления отдельными данными .
